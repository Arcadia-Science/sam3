...
def load_dummy_video(image_size, offload_video_to_cpu, num_frames=60, do_zeros=False, compute_device=None):
    """
...
class AsyncVideoFileLoaderWithTorchCodec:
    ...
    def __init__(
        ...
        if gpu_device is not None:
            accel_device = gpu_device
        elif torch.cuda.is_available():
            accel_device = torch.device("cuda", torch.cuda.current_device())
        else:
            accel_device = get_device()
        gpu_id = accel_device.index if accel_device.type == "cuda" else None
        if offload_video_to_cpu:
            out_device = torch.device("cpu")
        else:
            out_device = accel_device
        ...
        if gpu_acceleration:
            self.img_mean = self.img_mean.to(accel_device)
            self.img_std = self.img_std.to(accel_device)
            if accel_device.type == "cuda":
                decoder_option = {"device": str(accel_device)}
            else:
