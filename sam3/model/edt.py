...
"""Euclidean distance transform (EDT) – Triton kernel with CPU fallback."""
...
def _edt_triton_impl(data: torch.Tensor) -> torch.Tensor:
    """Triton implementation of batched EDT (requires CUDA + triton)."""
    assert data.dim() == 3
    assert data.is_cuda
    B, H, W = data.shape
...
def _edt_cpu_fallback(data: torch.Tensor) -> torch.Tensor:
    """CPU fallback using scipy for EDT."""
    try:
        from scipy.ndimage import distance_transform_edt
    except ImportError:
        raise ImportError(
            "scipy is required for CPU/MPS EDT fallback. Install with: pip install scipy"
        )
    ...
    return torch.stack(results).to(orig_device)
...
def edt_triton(data: torch.Tensor) -> torch.Tensor:
    ...
    Uses Triton on CUDA when available, falls back to scipy on CPU/MPS.
    ...
    if data.is_cuda and HAS_TRITON:
